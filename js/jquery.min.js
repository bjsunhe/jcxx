...
/*! jQuery plugins nextprev 图片横向按钮循环切换 */
;(function(jQuery) {
	$.fn.nextprev = function(options){
...
;(function(jQuery) {
	$.fn.iscroll = function(options){
		...
		ul.css('width', itemWidth*itemCount);

		var hl = '';
		hl += '<ul class="'+opts.pgClass+'">';
		for(var i = 1; i <= itemCount; i++){
			hl += '<li'+(i==1?' class="cur"':'')+'>'+i+'</li>';
		}
		hl += '</ul>';
		ul.after(hl);

		var pg = $this.find('.'+opts.pgClass);
		...
		function AutoScroll(i){
			ul.stop(true,false).animate({left:-itemWidth*i}, opts.speed);
			pg.find("li").eq(i).addClass("cur").siblings().removeClass();
		};
...
(function($) {
	$.fn.marquee = function(conf) {
		var o = $.extend({
			speed: parseInt($(this).attr('speed')) || 10,
			step: parseInt($(this).attr('step')) || 1,
			direction: $(this).attr('direction') || 'left',
			pause : $(this).attr('pause') || 0,
			type: $(this).attr('type') || 'normal' // pasue
		}, conf || {});

		var dIndex = jQuery.inArray(
			o.direction,
			['right', 'down']
		);

		if (dIndex > -1) {
			o.direction = ['left', 'up'][dIndex]; // 换算成left,up
			o.step = -o.step;
		};

		var timer;
		var div = $(this);
		var divWidth = div.innerWidth(); // wrap的宽度
		var divHeight = div.innerHeight(); // wrap的高度
		var ul = $("ul", div);
		var li = $("li", ul);
		var lisize = li.size();
		...
		}else if(o.type == 'pasue'){
			width = li.outerWidth() * lisize;
			height = li.outerHeight() * lisize;
		}else{
			return;
		}
		...
		if ((o.direction == 'left' && width > divWidth) || (o.direction == 'up' && height > divHeight)) {
			...
			// 按照speed来执行_marquess
			timer = setInterval(_marquee, o.speed);
		...
		function _marquee() {
			if (o.direction == 'left') {
				...
				if (o.type == 'pasue' && (l+1) % (width / lisize) == 0) {
					_pause();
				}
			} else {
				var t = div.scrollTop();
				...
				if (o.type == 'pasue' && (t+1) % (height / lisize) == 0) {
					_pause();
				}
			}
		}
...
/* jQuery plugins artDialog 5.0.4 */
